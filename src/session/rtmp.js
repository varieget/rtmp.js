...
class NodeRtmpSession {
  constructor(config, socket) {
    ...
    this.pingTime = config.ping ? config.ping * 1000 : RTMP_PING_TIME;
  ...
  stop() {
    if (this.isStarting) {
      ...
      Logger.log(`[rtmp disconnect] streamId=${this.publishStreamId}`);
  ...
  reject() {
    Logger.log(`[rtmp reject] streamId=${this.publishStreamId}`);
    this.stop();
  }
  ...
  rtmpAudioHandler() {
    ...
    if (this.audioCodec == 0) {
      ...
      if (sound_format != 10 && sound_format != 13) {
        Logger.log(
          `[rtmp publish] Handle audio. streamPath=${this.publishStreamPath} sound_format=${sound_format} sound_type=${sound_type} sound_size=${sound_size} sound_rate=${sound_rate} codec_name=${this.audioCodecName} ${this.audioSamplerate} ${this.audioChannels}ch`
        );
      }
    }
  ...
  rtmpVideoHandler() {
    ...
    if (this.videoCodec == 0) {
      ...
      Logger.log(
        `[rtmp publish] Handle video. streamPath=${this.publishStreamPath} frame_type=${frame_type} codec_id=${codec_id} codec_name=${this.videoCodecName} ${this.videoWidth}x${this.videoHeight}`
      );
    }
  ...
  rtmpDataHandler() {
    let offset =
      this.parserPacket.header.type === RTMP_TYPE_FLEX_STREAM ? 1 : 0;
    let payload = this.parserPacket.payload.slice(
      offset,
      this.parserPacket.header.length
    );
  ...
  onConnect(invokeMessage) {
    ...
    Logger.log(
      `[rtmp connect] ip=${this.ip} app=${this.appname} args=${JSON.stringify(
  ...
  onPublish(invokeMessage) {
    ...
    if (context.publishers.has(this.publishStreamPath)) {
      this.reject();
      Logger.log(
        `[rtmp publish] Already has a stream. streamPath=${this.publishStreamPath} streamId=${this.publishStreamId}`
      );
      this.sendStatusMessage(
      ...
      Logger.log(
        `[rtmp publish] NetConnection is publishing. streamPath=${this.publishStreamPath} streamId=${this.publishStreamId}`
      );
      this.sendStatusMessage(
        this.publishStreamId,
        'error',
      ...
      Logger.log(
        `[rtmp publish] New stream. streamPath=${this.publishStreamPath} streamId=${this.publishStreamId}`
      );
  ...
  onPlay(invokeMessage) {
    ...
    if (this.isPlaying) {
      Logger.log(
        `[rtmp play] NetConnection is playing. streamPath=${this.playStreamPath}  streamId=${this.playStreamId} `
      );
      this.sendStatusMessage(
        this.playStreamId,
        'error',
    ...
    if (context.publishers.has(this.playStreamPath)) {
      ...
      Logger.log(
        `[rtmp play] Stream not found. streamPath=${this.playStreamPath}  streamId=${this.playStreamId}`
      );
      this.isIdling = true;
  ...
  onStartPlay() {
    ...
    Logger.log(
      `[rtmp play] Join stream. streamPath=${this.playStreamPath} streamId=${this.playStreamId} `
    );
  }
  ...
  onPause(invokeMessage) {
    ...
    Logger.log(
      `[rtmp play] ${d} stream. streamPath=${this.playStreamPath} streamId=${this.playStreamId} `
    );
    if (!this.isPause) {
      this.sendStreamStatus(STREAM_BEGIN, this.playStreamId);
      if (context.publishers.has(this.playStreamPath)) {
  ...
  onReceiveAudio(invokeMessage) {
    ...
    Logger.log(`[rtmp play] receiveAudio=${this.isReceiveAudio}`);
  }

  onReceiveVideo(invokeMessage) {
    this.isReceiveVideo = invokeMessage.bool;
    Logger.log(`[rtmp play] receiveVideo=${this.isReceiveVideo}`);
  }
  ...
  onDeleteStream(invokeMessage) {
    if (invokeMessage.streamId == this.playStreamId) {
      ...
      Logger.log(
        `[rtmp play] Close stream. streamPath=${this.playStreamPath} streamId=${this.playStreamId}`
      );
      if (this.isStarting) {
        this.sendStatusMessage(
          this.playStreamId,
    ...
    if (invokeMessage.streamId == this.publishStreamId) {
      if (this.isPublishing) {
        Logger.log(
          `[rtmp publish] Close stream. streamPath=${this.publishStreamPath} streamId=${this.publishStreamId}`
        );
